...
export default function FirstHalfMonth({ schedule, type }: Props) {
  ...
  return (
    <table
      id={`${isDownload ? `first-half-month-${schedule.schedule_id}` : ''}`}
    ...
    >
      ...
      <tbody>
        ...
        <tr>
          <th className={`text-center ${downloadCellClass}`} colSpan={19}>
            服務項目
          </th>
        </tr>
        ...
        <tr>
          <th className={`text-center py-12 `} colSpan={4}>
            <div className="flex justify-center items-center">案主簽名</div>
          </th>
          {Array.from({ length: 15 }, (_, i) => i + 1).map((_, i) => (
            <td key={i} className={`text-center `} colSpan={1}></td>
          ))}
